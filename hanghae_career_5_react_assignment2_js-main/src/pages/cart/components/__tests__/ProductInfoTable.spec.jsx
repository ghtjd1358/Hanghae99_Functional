...
it('장바구니에 포함된 아이템들의 이름, 수량, 합계가 제대로 노출된다', async () => {
  ...
  expect(within(firstItem).getByText('3')).toBeInTheDocument(); // 수량 확인
  expect(within(firstItem).getByText('2,427')).toBeInTheDocument(); // 합계 (809 * 3)

  // Assert: 두 번째 아이템의 이름, 수량, 합계 금액을 확인합니다.
  expect(within(secondItem).getByText('Awesome Concrete Shirt')).toBeInTheDocument();
  expect(within(secondItem).getByText('4')).toBeInTheDocument(); // 수량 확인
  expect(within(secondItem).getByText('1,768')).toBeInTheDocument(); // 합계 (442 * 4)
...
it('특정 아이템의 수량이 변경되었을 때 값이 재계산되어 올바르게 업데이트 된다', async () => {
  ...
  await waitFor(() => {
    expect(within(firstItem).getByText('4,045')).toBeInTheDocument(); // 합계 금액 (809 * 5)
  });
});
...
it('특정 아이템의 삭제 버튼을 클릭할 경우 해당 아이템이 사라진다', async () => {
  ...
  expect(within(secondItem).getByText('Awesome Concrete Shirt')).toBeInTheDocument();
  // Act: 삭제 버튼을 클릭합니다.
  const deleteButton = within(secondItem).getByRole('button', { name: /삭제/i });
  await user.click(deleteButton);

  // Assert: 삭제 후 해당 아이템이 화면에서 사라졌는지 확인합니다.
  await waitFor(() => {
    expect(within(secondItem).queryByText('Awesome Concrete Shirt')).not.toBeInTheDocument();
  });
